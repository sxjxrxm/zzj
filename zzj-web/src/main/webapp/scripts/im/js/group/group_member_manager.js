...
//读取群组成员
var getGroupMemberInfo = function () {
    ...
    webim.getGroupMemberInfo(
        ...
        function (resp) {
            ...
            for (var i in resp.MemberList) {
                ...
                if (account !== userId) {
                	memberAccountList.push(account);
                }
            ...
            //$('#shutUpUntilInput').val(new Date(resp.MemberList[0].ShutUpUntil * 1000).Format("yyyy-MM-dd hh:mm:ss"));
...
var forbidSendMsg = function () {
    if (!webim.Tool.validNumber($('#fsm_shut_up_time').val())) {
        alert('您输入的禁言时间非法,只能是数字(0-525600)');
        return;
    }
    var shut_up_time = parseInt($('#fsm_shut_up_time').val()) * 60;
    if (shut_up_time > 525600) {
        alert('您输入的禁言时间非法,只能是数字(0-525600)');
        return;
    }
    var shut_up_until = '-';
    ...
    if (memberAccountList.length === 0) {
    	options = {
    	 ...
    	 options = {
    		        'GroupId': $('#fsm_group_id').val(),//群id
    		        'Members_Account': memberAccountList,//被禁言的成员帐号列表
    		        'ShutUpTime': shut_up_time//禁言时间，单位：秒
    		};
    }
    ...
    webim.forbidSendMsg(
        ...
        function (resp) {
            ...
            alert('设置成员禁言时间成功');
        	$('#shutUpUntilInput').text("禁言截止时间: "+shut_up_until);
        },
        function (err) {
            alert(err.ErrorInfo);
        }
