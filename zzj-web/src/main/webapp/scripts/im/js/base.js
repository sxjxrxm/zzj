...
//初始化群组成员信息****
function initGetMyGroupMemberInfo(){
	// 获取群成员id
	var accountArray = [];
	var options = {
            'GroupId': $("#roomId").val(),
	...
	webim.getGroupMemberInfo(options,function (resp) { // 1.先获得当前群成员id
		for(var j in resp.MemberList){
			accountArray.push(resp.MemberList[j].Member_Account)
		}
	    ...
	    var options1 = {
	        'To_Account': accountArray,
	        'TagList': tag_list
	    };
	    var nick;

	    webim.getProfilePortrait(options1, function(resp1) {// 2.根据成员id获得用户名
	    	for (var k in resp1.UserProfileItem) {
	    		nick = resp1.UserProfileItem[k].ProfileItem[0].Value;
	    		fromAccount = resp1.UserProfileItem[k].To_Account;
	    		
	    		memberMap[fromAccount]=nick;
	    	}
	    },
	    function(err1) {
	        alert(err1.ErrorInfo);
	    });
